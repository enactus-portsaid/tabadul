  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {(listings as Array<Record<string, unknown>>).slice(0, 6).map((item) => (
            <Link key={String(item.id)} href={`/${locale}/marketplace/${item.id}`}>
              <Card className="group overflow-hidden transition-all hover:shadow-md">
                ...
                <CardContent className="p-4">
                  <h3 className="truncate text-sm font-semibold text-text-primary group-hover:text-primary">
                    {String(item.title ?? 'Untitled')}
                  </h3>
                  <p className="mt-1 text-xs text-text-secondary">
                    {String(item.category ?? '')} · {String(item.location ?? '')}
                  </p>
        ...
        <Card>
          <CardContent className="py-16 text-center">
            <Bookmark className="mx-auto h-10 w-10 text-text-muted" />
